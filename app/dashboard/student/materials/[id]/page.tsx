...
import { getMaterialDetail, skillMap } from "@/lib/materials";
...
export default async function StudentMaterialDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell title={material.title} subtitle={`${skillMap[material.skill].label} • ${material.level} • ${material.theme}`}>
      ...
      {material.quizId ? <QuizRenderer quizId={material.quizId} questions={material.questions} /> : <section className="card panel"><h3>Belum ada kuis</h3><p className="muted-text">Materi ini belum memiliki kuis.</p></section>}
    </AppShell>
  );
}
